...
def plot_ratio_with_envelope(file_nuclear, file_baseline, hist_name, plot_key, xlabel, xlim, ylim, rebin, ylabel, outpath):
    ...
    legend.AddEntry(band_nuclear, "OO nuclear - TUJU21 - A=16, Z=8", "lf")
    legend.AddEntry(band_baseline, "OO baseline - TUJU21 - A=1, Z=1", "lf")
    ...
    fig.savefig(os.path.join(save_dir, f"npdf_tuju21_ratio_{plot_key}.pdf"))
...
def main():
    ...
    file_nuclear = root.TFile(path + "analysis_HI_bbbar_OO_TUJU21_16_8.root")
    file_baseline = root.TFile(path + "analysis_HI_bbbar_OO_TUJU21_16_8_pp_reference.root")
