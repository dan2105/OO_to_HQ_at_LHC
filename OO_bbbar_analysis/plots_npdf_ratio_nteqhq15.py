...
def plot_ratio_with_envelope(file_nuclear, file_baseline, hist_name, plot_key, xlabel, xlim, ylim, rebin, ylabel, outpath):
    ...
    legend.AddEntry(band_nuclear, "OO nuclear (nCTEQ15HQ)", "lf")
    ...
    fig.savefig(os.path.join(save_dir, f"npdf_ncteq15hq_ratio_{plot_key}.pdf"))
...
def main():
    ...
    file_nuclear = root.TFile(path + "analysis_HI_bbbar_OO_nCTEQ15HQ_FullNucleous_16_8.root")
    file_baseline = root.TFile(path + "analysis_HI_bbbar_OO_nCTEQ15HQ_FullNucleous_16_8_pp_reference.root")
